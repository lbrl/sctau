/**
 * @file SCTau.cpp
 * @brief Implementation of the SCTau detector
 */
#include "SCTau.h"
...
SCTau::SCTau(std::shared_ptr<const Calorimeter> ecal,
...
bool SCTau::electronAcceptance(const Track& track) const {
  return track.p3().Mag() > m_electronAcceptanceMagnitude && fabs(track.p3().Eta()) < m_electronAcceptanceEta;
}

double SCTau::electronEnergyResolution(const Particle& ptc) const {
  return m_electronEnergyFactor / sqrt(ptc.e());  // default factor = 0.1
}

bool SCTau::muonAcceptance(const Track& track) const {
...
double SCTau::muonResolution(const Particle& ptc) const {
  return m_muonResolution;  // default 0.02;
}

SCTau CreateDefaultSCTau() {
  ...
  auto ecal = std::make_shared<const SCTauECAL>(1.3,     // innerRadius
                                              2,       // innerZ
                                              1.55,    // outerRadius
                                              2.1,     // outerZ
                                              8.9e-3,  // x0
  ...
  auto hcal = std::make_shared<const SCTauHCAL>(1.9,   // innerRadius
                                              2.6,   // innerZ
                                              2.9,   // outerRadius
                                              3.6,   // outerZ
                                              0.2,   // clusterSize
  ...
  auto tracker = std::make_shared<const SCTauTracker>(1.29,    // outerRadius= 1.29
                                                    1.99,    // outerZ  = 1.99
                                                    0,       // x0 = 0,
                                                    0,       // lambdaI = 0,
  ...
  auto field = std::make_shared<const SCTauField>(3.8,  // field magnitude
                                                2.9,  // outerRadius
                                                3.6   // outerZ
                                                );

  return SCTau(ecal, hcal, tracker, field,

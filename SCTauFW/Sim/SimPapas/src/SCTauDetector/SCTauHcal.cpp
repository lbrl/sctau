...
SCTauHCAL::SCTauHCAL(double innerRadius, double innerZ, double outerRadius, double outerZ, double clusterSize, double x0,
...
double SCTauHCAL::clusterSize(const Particle& ptc) const {
  (void)ptc;             // suppress warning messages for unused parameters;
  return m_clusterSize;  // default 0.2
}

bool SCTauHCAL::acceptance(const Cluster& cluster) const {
    /*
...
double SCTauHCAL::energyResolution(double energy, double eta) const {
  int part = kBarrel;
...
double SCTauHCAL::energyResponse(double energy, double eta) const {
  int part = kBarrel;
  if (fabs(eta) > m_etaCrack) part = kEndCap;

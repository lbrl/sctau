...
PapasAlg::PapasAlg(const std::string& name, ISvcLocator* svcLoc) :
...
StatusCode PapasAlg::initialize() {
    ...
    // curiously does not work if joined with following line
    SmartIF<IPapasDetSvc> papasDetSvc;
    papasDetSvc = service(m_detServiceName);
    if (!papasDetSvc) {
        error() << "Unable to locate Papas Detector Service "
                << m_detServiceName << endmsg;
    ...
    // nb second value is dummy & not used at present
    papaslog::papaslogger =
        papaslog::getDefaultLogger("PapasAlg", papaslog::Logging::ERROR);
...
StatusCode PapasAlg::execute() {
    ...
    try {
        ...
        for (auto atool : m_tools) {
            // run each tool
            atool->run(pevent, m_spDetector);
        }
        m_exportTool->run(pevent, m_particleLinks);
        // summary details of what the event contains
        debug() << "PAPAS event contains " << std::endl
                << pevent.info() << endmsg;
        ...
        // in case of problems
        warning() << "Event :" << m_eventno
                  << "Exception was thrown from gaudi papas tool"
                  << message << endmsg;
        debug() << "PAPAS event contains " << std::endl
                << pevent.info() << endmsg;
        m_importTool->clear();

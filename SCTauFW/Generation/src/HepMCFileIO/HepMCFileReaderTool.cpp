...
HepMCFileReader::HepMCFileReader(const std::string& type,
...
StatusCode HepMCFileReader::initialize() {
    ...
    m_file = std::make_unique<HepMC::IO_GenEvent>(
            m_filename.value().c_str(), std::ios::in);
    // check that readable
...
StatusCode HepMCFileReader::getNextEvent(HepMC::GenEvent& event) {
    if (!m_file->fill_next_event(&event)) {
        ...
        error() << "Premature end of file: Please set the number\
                    of events according to hepMC file." << endmsg;

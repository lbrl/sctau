...
DECLARE_TOOL_FACTORY(ParticleGun)

/// Constructor
ParticleGun::ParticleGun(const std::string& type,
...
StatusCode ParticleGun::initialize() {
    ...
    info() << "Momentum range: " << m_minMom / Gaudi::Units::GeV
           << " GeV <-> " << m_maxMom / Gaudi::Units::GeV << " GeV"
           << endmsg;
    info() << "Theta range: " << m_minTheta / Gaudi::Units::rad
           << " rad <-> " << m_maxTheta / Gaudi::Units::rad << " rad"
           << endmsg;
    info() << "Phi range: " << m_minPhi / Gaudi::Units::rad 
           << " rad <-> " << m_maxPhi / Gaudi::Units::rad << " rad"
           << endmsg;
...
void ParticleGun::generateParticle(Gaudi::LorentzVector& momentum,
...
StatusCode ParticleGun::getNextEvent(HepMC::GenEvent& theEvent) {
    ...
    HepMC::GenParticle* p = new HepMC::GenParticle(
        HepMC::FourVector(theFourMomentum.Px(),
                          theFourMomentum.Py(),
                          theFourMomentum.Pz(),
                          theFourMomentum.E()),
                          thePdgId, 1);  // hepmc status code for final state particle

...
HepMCHistograms::HepMCHistograms(const std::string& name,
                                 ISvcLocator* svcLoc) :
...
StatusCode HepMCHistograms::initialize() {
    ...
    m_eta = new TH1F("GenEta", "Generated particles Pseudorapidity",
                     100, -10, 10);
...
StatusCode HepMCHistograms::execute() {
    ...
    info() << "Processing event with " << evt->particles_size()
           << " particles" << endmsg;

    for (HepMC::GenEvent::particle_const_iterator it = evt->particles_begin(),
         end = evt->particles_end(); it != end; ++it) {
        auto particle = *it;
        m_eta->Fill(particle->momentum().eta());
        m_pt->Fill(particle->momentum().perp());
    }

    for (HepMC::GenEvent::vertex_const_iterator it = evt->vertices_begin(),
         end = evt->vertices_end(); it != end; ++it) {
        auto vertex = *it;

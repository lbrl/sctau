...
HepMCToEDMConverter::HepMCToEDMConverter(const std::string& name,
                                         ISvcLocator* svcLoc) :
    GaudiAlgorithm(name, svcLoc) {
    declareProperty("hepmc", m_hepmchandle,
                    "HepMC event handle (input)");
    declareProperty("genparticles", m_genphandle,
                    "Generated particles collection (output)");
    declareProperty("genvertices", m_genvhandle,
                    "Generated vertices collection (output)");
}
...
StatusCode HepMCToEDMConverter::execute() {
    ...
    // First cover the case that hepMC file is not 
    // in expected units and then convert to EDM default
    const double hepmc2EdmLength =
      HepMC::Units::conversion_factor(event->length_unit(),
          gen::hepmcdefault::length) * gen::hepmc2edm::length;
    const double hepmc2EdmEnergy =
      HepMC::Units::conversion_factor(event->momentum_unit(),
    ...
    std::unordered_map<const HepMC::GenVertex*,
          fcc::GenVertex> hepmcToEdmVertexMap;
    // temp variable for the transfer of position / momentom
    ...
    for (auto particle_i = event->particles_begin();
        ...
        if(std::find(m_hepmcStatusList.begin(),
                     m_hepmcStatusList.end(),
                     (*particle_i)->status()) == m_hepmcStatusList.end() &&
           m_hepmcStatusList.size() > 0) continue;
        // create edm 
        ...
        // create production vertex, if it has not already been
        // created and logged in the map
        HepMC::GenVertex* productionVertex =
                     (*particle_i)->production_vertex();
        if (nullptr != productionVertex) {
            if (hepmcToEdmVertexMap.find(productionVertex) !=
                hepmcToEdmVertexMap.end()) {

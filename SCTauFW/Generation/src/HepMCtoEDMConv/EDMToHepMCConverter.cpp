...
EDMToHepMCConverter::EDMToHepMCConverter(const std::string& name,
                                         ISvcLocator* svcLoc) :
    GaudiAlgorithm(name, svcLoc) {
    declareProperty("hepmc", m_hepmchandle,
                    "HepMC event handle (output)");
    declareProperty("genparticles", m_genphandle,
                    "Generated particles collection (input)");
    declareProperty("genvertices", m_genvhandle,
                    "Generated vertices collection (input)");
}

StatusCode EDMToHepMCConverter::initialize() {
...
StatusCode EDMToHepMCConverter::execute() {
    ...
    double hepmc2EdmLength =
        ...
        gen::hepmc2edm::length;
    double hepmc2EdmEnergy =
        conversion_factor(event->momentum_unit(), gen::hepmcdefault::energy) *
        gen::hepmc2edm::energy;
    for (auto p : *(particles)) {
        if (p.status() == 1) {  // only final state particles
            GenParticle* pHepMC =
            new GenParticle(
                HepMC::FourVector(p.p4().px, p.p4().py, p.p4().pz,
                                  p.p4().mass / hepmc2EdmEnergy),
                                  p.pdgId(), p.status());
            // hepmc status code for final state particle
            fcc::ConstGenVertex vStart = p.startVertex();
            if (p.startVertex().isAvailable()) {
                HepMC::GenVertex* v =
                    new HepMC::GenVertex(
                       HepMC::FourVector(vStart.position().x / hepmc2EdmLength,
...
StatusCode EDMToHepMCConverter::finalize() {

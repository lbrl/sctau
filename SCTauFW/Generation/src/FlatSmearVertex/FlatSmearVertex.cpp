...
StatusCode FlatSmearVertex::initialize() {
    ...
    if (m_zDir == -1) {
        ...
        return Error("BeamDirection can only be set to\
                      -1 or 1, or 0 to switch off TOF");
    }

    info() << "Smearing of interaction point with flat distribution "
           << " in x, y and z " << endmsg;
    info() << infoMsg << endmsg;
    info() << " with " << m_xmin / Gaudi::Units::mm << " mm <= x <= "
           << m_xmax / Gaudi::Units::mm << " mm, "
...
StatusCode FlatSmearVertex::smearVertex(HepMC::GenEvent& theEvent) {
    ...
    for (auto vit = theEvent.vertices_begin();
         vit != theEvent.vertices_end(); ++vit) {
        Gaudi::LorentzVector pos((*vit)->position());
        pos += dpos;
        (*vit)->set_position(
                HepMC::FourVector(pos.x(), pos.y(), pos.z(), pos.t()));
    }
